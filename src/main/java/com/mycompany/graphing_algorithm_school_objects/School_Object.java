 ...
 * @author stmar
 */
public class School_Object implements Comparable<School_Object> {
...

    {
        return String.format(
                "Below are the following characteristic of the school: %nSchool Name: %s%nSchool Student Type: %s%nSchool Classification type:"
                        + " %s%nSchool Enrollment Size: %d",
                getSchool_name(), getStudent_Type(), getSchool_classification(), getEnrollment_Size());
    }

    @Override
    public int compareTo(School_Object School_two) {
        return School_two.getEnrollment_Size() - this.getEnrollment_Size();
    }
